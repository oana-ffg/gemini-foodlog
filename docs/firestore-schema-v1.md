...
| `feedback/{feedback_id}` | Immutable user confirmation/correction/discard | optional `meal_id`/`question_id`, `kind`, legacy whole-meal actual value, optional exact correction target and base revision, explanation <= 2,000 chars, optional explicit learning disposition (`reusable` or `insufficient_information`), and `idempotency_hash`; never overwrites the original inference. Reusable disposition requires both a replacement and explanation; the backend does not infer it from prose. |
