...
| `event_heads/{camera_id}` and `event_heads/__account__` | Transactional temporal-grouping pointers | latest event ID for one account camera or across the account plus update time; internal state only, never agent evidence. The account pointer lets temporally related evidence from different cameras join one event without any global or cross-tenant query. |
