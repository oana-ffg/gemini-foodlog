...
| `event_heads/{camera_id}` | Transactional temporal-grouping pointer | latest event ID for that account camera and update time; internal state only, never agent evidence. |
