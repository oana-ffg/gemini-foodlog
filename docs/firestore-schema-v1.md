...
| `identities/{firebase_uid}` | Login-to-account lookup and admission evidence | `account_id`, `account_class` (`public` or explicitly configured `internal`), immutable `admission_email_normalized`, `admission_email_verified`, and `status` (`active` or `capacity_reclaimed`); one document per Firebase UID. Public admission persists the email evidence supplied by the verified Firebase token in the same transaction as the account and capacity counter. A later verified-email change does not change ownership or overwrite the original admission evidence. |
...
| `waitlist/{sha256_firebase_uid}` | Capacity overflow and product-interest list | While active: verified `email_normalized`, `reason` (`capacity`), `mailing_list_opt_in` (`true`), `policy_version`, and timestamps. Withdrawal sets `status=withdrawn`; successful admission sets `status=fulfilled` and records the admitted account/time. Both terminal states clear the email and opt-in. The raw Firebase UID is never stored. One document per verified identity is accepted or reactivated only while public capacity is full. |

## Account root and bounded counters

`accounts/{account_id}` stores only bounded account metadata: `owner_user_id`,
`status` (`active` or operator-set `capacity_reclaimed`), `timezone`, and
consent/mailing-list summaries. Growing lists are always
subcollections.
...
| `capacity_operations/{sha256_operation}` | Immutable public-slot lifecycle evidence | operator-supplied UUID, exact account/owner, `reclaim` or `restore`, bounded reason, prior/resulting status, resulting active count and configured limit, and timestamp. The same transaction updates account, identity, global capacity, and an account-scoped audit event. |
...
| `events/{event_id}` | Multi-frame kitchen activity | `camera_ids` <= 8, `status` (`open`, `inferred`, or `user_classified`), current subject revision, first/last capture timestamps, `capture_count`, nullable `meal_id`, grouping-policy version; frame IDs are queried from captures rather than accumulated in an array. Open events are owner-visible journal items even before a model result exists. |
...
| `meals/{meal_id}/revisions/{revision_id}` | Immutable inference/correction/disposition history | revision number, source (`inference`, `user_feedback`, or `user_classification`), status, complete bounded inference snapshot, optional `feedback_id` or `classification_id`, `base_revision_number`, and discriminated correction target (`meal`, `component`, `ingredient`, or `preparation_method`); unique revision number per meal. A `not_cooking` revision retains the prior inference, and explicit reclassification appends rather than overwrites. Targeted corrections require the current base revision and preserve every field outside their exact path. |
| `event_classifications/{sha256_key}` | Immutable owner classification before a model result | Event/meal ID, exact kind (`meal` or `not_cooking`), optional exact meal title and explanation, expected event revision, creation time, and deterministic request identity without the raw idempotency key. The same transaction creates the first meal revision, marks captures processed, changes the event to `user_classified`, and completes any pending, leased, or failed inference job so a late worker lease cannot publish over the owner. |
...
| `purchases/{purchase_id}` | One purchase-evidence lifecycle | merchant, explicit origin (`authenticated_email` or `synthetic_evaluation`), bounded `revision_count`, latest confirmation/final document IDs, and creation/update timestamps; order/invoice aliases and source documents remain separate so the root never grows arrays. Synthetic evaluation records are operator-seeded test evidence and are never represented as authenticated retailer history. |
| `purchase_identities/{sha256_merchant_origin_kind_reference}` | Exact business-identity alias | purchase ID, merchant/origin namespace, kind (`order` or `invoice`), reference hash, creation time; aliases are account-scoped, contain no plaintext reference, and conflicting aliases or origins never trigger an automatic merge. Existing authenticated-email aliases retain their historical identity hash. |
| `purchase_documents/{raw_mail_id}` | One immutable normalized source-document identity | purchase ID, raw-mail/content hashes, merchant, evidence origin, document kind, revision number, and optional normalized retailer-labelled order/invoice references; an exact transport retry returns this document while changed interpretation or synthetic timestamp conflicts. The authenticated path requires a content-bound DKIM verdict; the separate internal evaluation seeder requires the explicit synthetic origin and never creates a raw-mail authentication record. |
