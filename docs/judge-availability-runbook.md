...
The official [hackathon rules](https://allthingsagentichackathon.devpost.com/rules) make a hosted project URL optional but strongly encouraged. They explicitly allow the app to be offline at submission or judging when the video and repository clearly prove the Google Cloud deployment. Keeping FoodLog live remains a judge-experience goal, not an eligibility requirement, and it must not override the zero-out-of-pocket safety boundary.
...
4. **If no post-2026-09-24 funding is approved:** stop before the credit expires and follow the reviewed shutdown option. The public demo and repository remain the required deployment proof; do not claim the hosted app is still available.
5. **If the project remains funded:** use the later binding-rules date and run a signed-out URL check plus a judge-account read-only smoke on 2026-10-01.

## Current decision

Full zero-out-of-pocket availability through the later possible end of judging is not yet guaranteed because the known promotion ends first. That is no longer a contest-submission blocker: the live official requirement accepts clear video and repository deployment proof without continuous uptime. REL-002 therefore waits only for the consolidated human judge-path test; the pre-expiry funding or shutdown decision remains mandatory for billing safety.
